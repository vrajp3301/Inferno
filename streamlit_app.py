...
def calculate_retirement_savings(current_age, retirement_age, current_savings, annual_contribution, annual_return, annual_inflation, adjust_for_inflation):
...
def main():
    ...
    if st.button("Calculate"):
        estimated_savings = calculate_retirement_savings(current_age, retirement_age, current_savings, annual_contribution, annual_return, annual_inflation, adjust_for_inflation)
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.bar(["Estimated Retirement Savings"], [estimated_savings])
        ax.set_title("Retirement Savings Projection")
        ax.set_xlabel("Metric")
        ax.set_ylabel("Amount (USD)")
        ax.ticklabel_format(style='currency', axis='y', currency='USD')
        st.pyplot(fig)
